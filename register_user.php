...
<head>
    ...
    <title>AI.Jobs || Registrasi Pencari Kerja</title>
...
<body>
    ...
    <div class="section">
        <div class="container">
            <h3>Registrasi Pencari Kerja</h3>
            <div class="box">
                <form action="" method="POST" enctype="multipart/form-data">
                    <input type="text" name="nama" class="input-control" placeholder="Nama Lengkap" required>
                    <textarea name="alamat" class="input-control" rows="10" placeholder="Alamat"></textarea>
                    <input type="text" name="telpon" class="input-control" placeholder="Nomor Telpon" required>
                    ...
                    <h3>Resume</h3><input type="file" name="resume" class="input-control" required>
                    <input type="text" name="no_rek" class="input-control" placeholder="Nomor Rekening" required>
                ...
                <?php
                    if(isset($_POST['submit'])) {
                        ...
                        $tmp_resumename = $_FILES['resume']['tmp_name'];

                        $type1 = explode('.', $filename);
                        $type2 = $type1[1];

                        $type3 = explode('.', $resumename);
                        $type4 = $type3[1];

                        $newname = 'foto'.time().'.'.$type2;
                        $newresumename = 'resume'.time().'.'.$type4;
                        ...
                        if(!in_array($type2 || $type4, $tipe_diizinkan)) {
                            echo '<script>alert("Format file harus jpg, jpeg, atau png")</script>';

                        }else {
                            move_uploaded_file($tmp_name, './user_img/'.$newname);
                            move_uploaded_file($tmp_resumename, './user_img/'.$newresumename);
